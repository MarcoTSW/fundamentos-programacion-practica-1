...
function miInformacion() {
  ...
  const nombre = "Marco David Parera Flores";
  const edad = 22;
...
function mayorDeTres(a, b, c) {
  ...
  if (a >= b && a >= c) {
    //'a' es mayor o igual que 'b' Y 'a' es mayor o igual que 'c'
    return a;    
  } else {
    // Si no fue 'a' ni 'b', tiene que ser 'c'
    return c;
  }
}
...
function clasificarEdad(edad) {
  ...
  if (edad <= 17) {
    ...
    //Si llegamos hasta aqui, sabemos que la edad es > 17,
    // asi que solo necesitaremos verificar si es <= 64
    return "adulto";
   } else {
    // Si la edad no fue ni <= 17 ni <=64, debe ser de 65 o más 
    return "mayor";
   }
}
...
function obtenerColumna(matriz, indiceColumna) {
  ...
  for (let i = 0; i < matriz.length; i++) {
    ...
    if (indiceColumna < filaActual.length) {
      const elemento = filaActual[indiceColumna];

      columna.push(elemento);
    }
  }
...
function transponer(matriz) {
  ...
  const numFilasOriginal = matriz.length;
  const numColumnasOriginal = matriz[0].length;

  for (let j = 0; j < numColumnasOriginal; j++) {
    const nuevaFila = [];
    for (let i = 0; i < numFilasOriginal; i++) {
